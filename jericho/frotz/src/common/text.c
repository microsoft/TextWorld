...
extern zword next_property (zword);
...
void get_text (enum string_type st, zword addr, char *s)
{
    zchar *ptr;
    long byte_addr;
    zchar c2;
      ...
      for (i = 10; i >= 0; i -= 5) {
        ...
        switch (status) {
          ...
          case 1: /* abbreviation */
            ptr_addr = h_abbreviations + 64 * (prev_c - 1) + 2 * c;
            LOW_WORD (ptr_addr, abbr_addr);
            get_text (ABBREVIATION, abbr_addr, s);
            status = 0;
            break;
          case 2: /* ZSCII character - first part */
            status = 3;
 ...
 * describe_object
 *
 * Prints a detailed description of an object.
 *
 */
void describe_object (zword object) {
  if (!object) return;

  zword obj_name_addr = object_name(object);
  print_char(' ');
  ...
  if (length <= 0) {
    print_string("ERROR: Object too Short!\n");
    return;
  }

  zword code = 0x94a5;
  LOW_WORD(obj_name_addr+1, code);
  if (code == 0x94a5) {
    print_string("ERROR: Object has no code!\n");
    return;
  }
  ...
  print_num(object);
  print_string(": ");
  decode_text(LOW_STRING, obj_name_addr+1);
  ...
  print_string(" Attr: ");
  for (i=0; i<4; ++i) {
    LOW_BYTE(obj_addr + i, attr);
    print_num(attr);
    print_string(" ");
  }
  zword parent = get_parent(object);
  print_string(" par");
  print_num(parent);

  zword child = get_child(object);
  print_string(" ch");
  print_num(child);

  print_string(" sib");
  zword sib = get_sibling(parent);
  print_num(sib);

  // Print Properties
  print_string(" Props: ");
  zword prop_addr = first_property(object);
  zbyte prop_value;
  LOW_BYTE(prop_addr, prop_value);
  print_num(prop_value);
  // Next prop
  while (1) {
    print_string(" ");
 ...
 * describe_tree
 *
 * Prints the full tree of objects starting from the given one.
 *
 */
void describe_tree (zword object, int indent) {
  int i;
  if (!object) return;
  ...
  zword child = get_child(object);
  describe_tree(child, indent + 4);

  zword sibling = get_sibling(object);
  describe_tree(sibling, indent);
}
