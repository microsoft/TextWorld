...
walkthrough = 's/w/n/w/get pumpkin/get leaves/n/give turnip to daisy/s/e/s/e/n/s/w'

solution = walkthrough.split('/')

def test_gold():
    env = FrotzEnv("../../TextWorld-baselines/games/gold.z5", seed=4)
    env.reset()
    for act in solution:
...
def test_max_score():
    env = FrotzEnv("../roms/gold.z5", seed=4)
...
def test_score_detection():
    env = FrotzEnv("../roms/gold.z5", seed=5)
    env.reset()
    for act in solution:
...
def test_inventory():
    ...
    assert 'wristwatch' in inv_names
