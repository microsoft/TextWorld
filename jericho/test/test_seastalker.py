...
walkthrough = 'john/smith/yes/turn on videophone/adjust videophone/pick up microphone/turn it on/ask bly about problem/ask bly about monster/wait/turn off microphone/yes/drop microphone onto workbench/n/turn on computestor/ask computestor about videophone/e/open door/e/x panel/fix circuit breaker/w/l/ask sharon about computestor'
solution = walkthrough.split('/')

def test_seastalker():
    env = FrotzEnv("../../TextWorld-baselines/games/seastalker.z3", seed=6)
    # env = FrotzEnv("../../text-agents/roms/seastalker.z3", seed=6)
    env.reset()
    for act in solution:
...
def test_max_score():
    env = FrotzEnv("../roms/seastalker.z3", seed=6)
    assert env.get_max_score() == 100

def test_load_save_file():
    fname = 'seastalker.qzl'
    env = FrotzEnv("../roms/seastalker.z3", seed=6)
    env.reset()
...
#     for act in solution[:3]:
#         env.step(act)
#     for idx, act in enumerate(solution[3:14]):
