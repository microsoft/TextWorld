...
def test_jewel():
    env = FrotzEnv("../../TextWorld-baselines/games/jewel.z5", seed=4)
    env.reset()
    for act in solution:
...
def test_max_score():
    env = FrotzEnv("../roms/jewel.z5", seed=4)
    assert env.get_max_score() == 90

def test_load_save_file():
    fname = 'jewel.qzl'
    env = FrotzEnv("../roms/jewel.z5", seed=4)
    env.reset()
    for act in intro.split('/'):
        env.step(act)
...
def test_score_detection():
    env = FrotzEnv("../roms/jewel.z5", seed=5)
    env.reset()
    for act in intro.split('/'):
        env.step(act)
    for act in solution:
