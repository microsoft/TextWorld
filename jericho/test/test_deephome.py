...
walkthrough = 'push mountain/sw/open cabinet/take shield, sword/sw/get torch/wait/wait/s/s/s/s'
solution = walkthrough.split('/')

def test_deephome():
    env = FrotzEnv("../../TextWorld-baselines/games/deephome.z5", seed=4)
    env.reset()
    for act in solution:
        loc = env.get_player_location()
        print("{} {} Loc {}-{}".format(act, env.step(act), loc.name, loc.num))
        print("Inv {}".format([i.name for i in env.get_inventory()]))

def test_max_score():
    env = FrotzEnv("../roms/deephome.z5", seed=4)
    assert env.get_max_score() == 300
...
def test_score_detection():
    env = FrotzEnv("../roms/deephome.z5", seed=5)
    env.reset()
    for act in solution:
