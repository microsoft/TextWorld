...
walkthrough = 'z/z/z/d/x writing/wake body/z/z/z/s/x vial/n/d/u/d/carry charles/unlock door with key/open door/s/sw/w/n/n/x carvings/x body/read note/take book/x book/s/s/e/ne/n/u/s/x vials/take mukhtar vial/n/put vial on table/read writing/x paper/z/z/z/z/d/sw/s/sw/w/w/s/throw key at cat/x slabs/move slab/d/d/n/n/e/e/x rack/take all from it/put red powder in kettle/put yellow powder in kettle/turn switch/paint sign/w/w/throw bottle at monster/w/s/shout/push stone/z/z/e/e/look under stove//unlock chest with brass key/open chest/take crowbar/w/w/push stone/s/s/u/n/u/n/e/e/x wall/open door/e/push lever/z/z/z/z/pull lever/w/s/s/z/z/z/z/z'
solution = walkthrough.split('/')

def test_temple():
    env = FrotzEnv("../../TextWorld-baselines/games/temple.z5", seed=4)
    env.reset()
    for act in solution:
...
def test_max_score():
    env = FrotzEnv("../roms/temple.z5", seed=4)
    assert env.get_max_score() == 35
...
def test_score_detection():
    env = FrotzEnv("../roms/temple.z5", seed=5)
    env.reset()
    for act in solution:
...
def test_inventory():
    env = FrotzEnv("../roms/temple.z5", seed=5)
    env.reset()
    for act in solution[:40]:
        env.step(act)
    inv = env.get_inventory()
    inv_names = [o.name for o in inv]
    assert 'wrought iron key' in inv_names
