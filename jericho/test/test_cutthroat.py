...
walkthrough = 'get up/wind watch/read note/open dresser/get key/open door/go out/close door/lock door/d/out/e/e/e/e/s/sit down/order breakfast/wait/order water/drink water/yes/eat food/get up/out/w/w/w/w/sw/sw/nw/wait/wait/wait/wait/wait/wait/wait/wait/wait/wait/wait/wait'
solution = walkthrough.split('/')

def test_cutthroat():
    env = FrotzEnv("../../TextWorld-baselines/games/cutthroat.z3", seed=6)
    env.reset()
    for act in solution:
        loc = env.get_player_location()
        print("{} {} Loc {}-{}".format(act, env.step(act), loc.name, loc.num))
        print("Inv {}".format(env.get_inventory()))


def test_max_score():
    env = FrotzEnv("../roms/cutthroat.z3", seed=6)
...
def test_move_detection():
    env = FrotzEnv("../roms/cutthroat.z3", seed=6)
    env.reset()
    for idx, act in enumerate(solution[:10]):
