...
walkthrough = 'x bed/look under bed/x key/x guage/x switch/open guage/push button/push switch/z/z/z/x guard/hit guard/unlock door with key/open door/s/e/s/x box/open box/take grenade/x it/n/w/w/w/s/x panels/x cupboard/open it/pull pin/put grenade in cupboard/z/z/x electronics/n/n'

solution = walkthrough.split('/')

def test_zenon():
    env = FrotzEnv("../../TextWorld-baselines/games/zenon.z5", seed=4)
    env.reset()
    for act in solution:
...
def test_max_score():
    env = FrotzEnv("../roms/zenon.z5", seed=4)
...
def test_score_detection():
    env = FrotzEnv("../roms/zenon.z5", seed=5)
    env.reset()
    for act in solution:
