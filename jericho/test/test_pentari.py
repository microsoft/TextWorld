...
walkthrough = 'N/N/IN/CITY/E/COVERT/S/SMASH SEAL/N/N/N/IN/GET SCROLL/OUT/S/E/N/GET TOWEL/W/W/GET DAGGER/N/OPEN TOWEL/FWOOSH/GET EMERALD/PUT EMERALD IN TOWEL/S/E/E/U/GET EMERALD/PUT EMERALD ON BOX/LOOK'

solution = walkthrough.split('/')

def test_pentari():
    env = FrotzEnv("../../TextWorld-baselines/games/pentari.z5", seed=4)
    env.reset()
    for act in solution:
...
def test_max_score():
    env = FrotzEnv("../roms/pentari.z5", seed=4)
    assert env.get_max_score() == 70
...
def test_score_detection():
    env = FrotzEnv("../roms/pentari.z5", seed=5)
    env.reset()
    for act in solution:
