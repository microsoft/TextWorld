...
walkthrough = 'Stand up/Again/Get gumball/N/X gnome/Ask George about bag/Ask him about Beetlebaum/Ask him about Frank/Ask him about gumball/W/Open closet/Get bag then X it/Open bag/E/X dresser/Open drawer/Get gloves then wear it/S/E/Open curtain/Get soap/Put soap on zipper/Open bag/Drop soap/Remove gloves then drop it/W/S/NE/Get bench/S/Drop bench/Stand on bench then get key/Exit/Unlock door with key then drop key/Open Door/S/Search clothes/Get pack/Open closet then get safe/N/NE/X pack then read label/Shake pack/Put pack on gum/Drop pack/Get gum/S'

solution = walkthrough.split('/')

def test_tryst():
    env = FrotzEnv("../../TextWorld-baselines/games/tryst205.z5", seed=4)
    env.reset()
    for act in solution:
...
def test_max_score():
    env = FrotzEnv("../roms/tryst205.z5", seed=4)
...
def test_score_detection():
    env = FrotzEnv("../roms/tryst205.z5", seed=5)
    env.reset()
    for act in solution:
...
def test_inventory():
    ...
    assert 'wad of gooey gum' in inv_names
