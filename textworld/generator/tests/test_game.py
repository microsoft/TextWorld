...
DATA = testing.build_complex_test_game()
...
class TestQuestProgression(unittest.TestCase):
    ...
    def test_completed(self):
        ...
        # Alternative winning strategy but with carrot in inventory.
        quest = QuestProgression(self.quest, KnowledgeBase.default())
        state = self._apply_actions_to_quest(self.eating_carrot.actions[:1], quest)  # Take carrot.
        self._apply_actions_to_quest(self.quest.win_event.events[1].actions, quest, state)
        assert quest.done
        assert quest.completed and not quest.failed
        assert quest.winning_policy is None
