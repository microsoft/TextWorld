...
class WorldEntity:
    ...
    def add(self, *entities: List["WorldEntity"]) -> None:
        ...
        if KB.types.is_descendant_of(self.type, "r"):
            name = "at"
        elif KB.types.is_descendant_of(self.type, ["c", "I"]):
            name = "in"
        elif KB.types.is_descendant_of(self.type, "s"):
            name = "on"
        else:
...
class WorldPath:
    ...
    def door(self, door: WorldEntity) -> None:
        if door is not None and not KB.types.is_descendant_of(door.type, "d"):
...
class GameMaker:
    ...
    def __init__(self) -> None:
        ...
        self.rooms = []
        self.paths = []
        self._types_counts = KB.types.count(State())
    ...
    def new(self, type: str, name: Optional[str] = None,
        ...
        """
        var_id = type
        if not KB.types.is_constant(type):
