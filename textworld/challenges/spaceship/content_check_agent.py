...
def play(agent, path, max_step=50, nb_episodes=10, verbose=True):
    """
        This code uses the agent design in the content check game.
...
start_time = time()
print(os.path.realpath("./games/contentCheck_levelEasy_white.ulx"))
play(agent, "./games/contentCheck_levelEasy_white.ulx", max_step=step_size, nb_episodes=200, verbose=False)
print("Trained in {:.2f} secs".format(time() - start_time))

print(' =====  Test  ========================================================= ')
agent.test(method='random')
play(agent, "./games/contentCheck_levelEasy_white.ulx", max_step=step_size)  # Medium level game.

save_path = "./model/contentCheck_levelEasy_whiteBox_random.npy"
