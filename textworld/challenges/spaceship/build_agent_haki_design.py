...
class CommandScorer(nn.Module):
    ...
    def forward(self, obs, commands, method='random'):
        ...
        # Compute score per command.
        action_state = F.relu(self.att_cmd(cmd_selector_input))#.squeeze(-1)  # 1 x Batch x cmds
...
class NeuralAgent:
    ...
    def act(self, obs: str, score: int, done: bool, infos: Mapping[str, Any], method: str) -> Optional[str]:
        ...
        outputs, indexes, values = self.model(input_tensor, commands_tensor, method=method)
...
def play(agent, path, method='random', max_step=50, nb_episodes=10, verbose=True):
    """
    ...
    for no_episode in range(nb_episodes):
        ...
        nb_moves = 0
        while not done:
            command = agent.act(obs, score, done, infos, method)
...
METHOD = ['random', 'arg-max', 'eps-soft', 'stochastic']
play(agent, "./games/levelMedium.ulx", method='eps-soft', nb_episodes=100, verbose=False)  # Medium level game.